...
class DataGenerator:
    def generate_set_of_metrics(self, start_time: str, steps_number: int, time_period: str):
        ...
        metrics['metric_2'] = (50 + np.cos(metrics['metric_1'].shift(5).bfill() / 20) * 30 +
                               np.random.normal(0, 2, len(primary_metric)))
        metrics['metric_3'] = (70 + np.sin(metrics['metric_1'].shift(7).bfill() / 10) * 40 +
                               np.random.normal(0, 3, len(primary_metric)))
        metrics['metric_4'] = (90 + np.log1p(metrics['metric_1'].shift(10).bfill()) * 20 +
                               np.random.normal(0, 1.5, len(primary_metric)))
        metrics['metric_5'] = (75 + np.sqrt(metrics['metric_1'].shift(4).bfill()) * 25 +
                               np.random.normal(0, 2, len(primary_metric)))
        metrics['metric_6'] = (100 + (metrics['metric_1'].shift(30).bfill() / 2) +
                               np.random.normal(0, 4, len(primary_metric)))
        metrics['metric_7'] = (30 + np.sin(metrics['metric_2'].shift(2).bfill() / 50) * 15 +
                               np.cos(metrics['metric_3'].shift(4).bfill() / 50) * 15 +
                               np.random.normal(0, 5, len(primary_metric)))
        metrics['metric_8'] = (np.exp(metrics['metric_1'].shift(1).bfill() / 100) +
                               np.random.normal(0, 1, len(primary_metric)))
        metrics['metric_9'] = (50 + np.clip(100 - metrics['metric_1'].shift(5).bfill(), 0, 100) +
                               np.random.normal(0, 3, len(primary_metric)))
        metrics['metric_10'] = (50 + np.abs(np.sin(metrics['metric_1'].shift(5).bfill() / 30) * 70) +
                                np.random.normal(0, 2, len(primary_metric)))

        # Inject unique anomalies into each dependent metric
        for metric_name in metrics.columns:
            if metric_name != 'metric_1':
                anomaly_flags[metric_name], anomaly_ranges[metric_name] = self._add_anomalies(metrics, metric_name)
    ...
    def _add_anomalies(self, metrics_df, metric_name, anomaly_rate=0.001, anomaly_magnitude=50, window_size=300):
        ...
        for idx in anomaly_indices:
            duration = np.random.randint(1, 120)  # Anomalies last between 1 and 120 seconds
            ...
            anomaly_values = (metrics_df.loc[idx:end_idx, metric_name] +
                              direction * np.random.normal(anomaly_magnitude,
                                                           15,
